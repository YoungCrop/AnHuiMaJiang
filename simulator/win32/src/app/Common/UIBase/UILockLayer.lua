...
local m = class("UILockLayer", function()
...
function m:setTouchBeganFunc(_func)
	self.m_touchBeganFunc = _func
end

function m:setTouchMovedFunc(_func)
	self.m_touchMoveFunc = _func
end

function m:setTouchEndedFunc(_func)
	self.m_touchEnedFunc = _func
end

function m:setCloseWithLayer(_layer,_func)
...
function m:setCloseWithRect(_rect , _func)
	print("m:setCloseRect::::",_rect.x, _rect.y,_rect.width,_rect.height)
	self.m_closeRecct = _rect
	self.m_closeFunc = _func
end

--弹出框 弹出效果
function m:showOpen(_fun)
	local callfunc = nil
	if _fun then
