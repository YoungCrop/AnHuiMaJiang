...
local NodeTopBarNew = require("app.Game.UI.UIIRoom.RoomComm.NodeTopBarNew")
...
--
    self.btnMap = {}
    self.NodeTopBarNew = NodeTopBarNew:create(self.param)
    self.NodeTopBarNew:addTo(self.csbNode)
    self:loadCSB()
end
...
n.nodeMap = {
    ...
    Spr_turnPosBg    = {"Spr_turnPosBg", false,},  -- 隐藏轮换位置标识（东南西北信息）
    Node_roundState  = {"Node_roundState", false,},  -- 隐藏牌局状态（倒计时，剩余牌局，剩余牌数）
    Label_playTimeCD = {"Label_playTimeCD", false,},  -- 倒计时
    mahjong_table    = {"mahjong_table", true,},  -- 背景
...
function m:loadCSB()
    ...
    if self.param.roomType == DefineRule.RoomType.BigMatch then
        self.btnMap["Voice_Btn"]:setVisible(false)
    ...
    self.btnMap["mahjong_table"]:setScale(math.max(1,display.height/CC_DESIGN_RESOLUTION.height))
...
function m:setTuoGuanBtnShow(isShow)
    self.NodeTopBarNew:setTuoGuanBtnVisible(isShow)
...
function m:changeGameBg()
    ...
    self.btnMap["mahjong_table"]:initWithFile(gameBgPic)
end

function m:setRoundIndex(str)
    if self.param.gameID == DefineRule.GameID.POKER_DDZ then
        self.NodeTopBarNew:setRoundIndex(str)
    else
        local s = string.sub(str,4,-4)--中文一个字占2个字节
        self.btnMap["txtRoundNum"]:setString(s)
        self.NodeTopBarNew:setRoundBtnHide()
    end
end
...
function m:setBtnBackLobby(isShow)
    self.NodeTopBarNew:setBtnBackLobby(isShow)
end
