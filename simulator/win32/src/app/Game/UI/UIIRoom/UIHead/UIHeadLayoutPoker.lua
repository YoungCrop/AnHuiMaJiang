...
function m:roomAddPlayer(sPos)
    local pInfo = gRoomData.sPosPlayers[sPos]
...
function m:setZuoLaPaoTxt(sPos,txtStr)
    ...
    log("setZuoLaPaoTxt--uipos--",i,txtStr)
...
function m:onRcvChatMsg(msgTbl)
    ...
    local pInfo = gRoomData.sPosPlayers[msgTbl.m_pos]
    if not pInfo then
...
function m:onShowBeatOthers(msgTbl)
    local srcInfo = gRoomData.sPosPlayers[msgTbl.m_srcPos]
    if not srcInfo then
        log("===---onShowBeatOthers===srcInfo")
        return
    end

    local destInfo = gRoomData.sPosPlayers[msgTbl.m_destPos]
