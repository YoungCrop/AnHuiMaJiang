...
local UISettlementFinal = require("app.Game.UI.UIGame.MJHuaiBei.UISettlementFinal")
local UISettlementOneRound = require("app.Game.UI.UIGame.MJHuaiBei.UISettlementOneRound")
...
local m = class("SceneMJHuaiBeiEx", gComm.SceneBase)
...
local resCsb = "Csd/Scene/SceneMJHuaiBeiNew.csb"
...
--是否显示听牌提示,只有弃(胡/自摸),且听时，才显示 听箭头
	...
	self.CtlMJNetListener = CtlMJNetListener:create(self,enterRoomMsgTbl.m_gameID)
...
function m:onRcvEnterRoom(msgTbl)
	dump(msgTbl,"m:onRcvEnterRoom---9999===")
	...
	self.playmes = {}
	self.playmes.m_playtype = msgTbl.m_playtype
	self.playmes.roomId = msgTbl.m_deskId

	self.gParam.roomPeopleNum = msgTbl.m_playerNum
end
...
function m:removePlayerForRoom()
	...
	for _, msgTbl in pairs(self.removePlayers) do
		...
		if roomPlayer then
			...
			-- 取消头像下载监听
			-- self.UIHeadLayout:removePlayer(msgTbl.m_pos)
...
function m:onRcvSyncMakeDecision(msgTbl)
	...
	dump(msgTbl,"m:onRcvSyncMakeDecision22--",self.gParam.sPosSelf)
...
function m:addMjTilePungEx(seatIdx, mjColor, mjNumber,pengServerPos)
	...
	op = gRoomData.opCodeEnum.SelfOp--淮北不需要标明左右杠、碰
...
function m:setZuoLaPao(param)
	-- zhuangPos = subMsg.m_zhuangPos,
	-- maNum1    = subMsg.m_maNum1,
	-- maNum2    = subMsg.m_maNum2,
	-- maState   = subMsg.m_maState,--0未开始买码 1 已经买好,2不变化
	-- isBroast  = subMsg.m_isBroast,--0，1

	if param.isBroast == 0 then
		self.UIRoomInfoBottom:setBtnVisible({btnInviteFriend = false})

		self.UIRoomInfoUpper:setBtnBackLobby(false)
		for i,v in ipairs(param.maState) do
			if v == 1 then
				self.UIHeadLayout:setZuoLaPaoTxt(i-1,"设置完毕")
			elseif v == 0 then
				self.UIHeadLayout:setZuoLaPaoTxt(i-1,"设置中")
			elseif v == 2 then
			end
		end
		if param.maState[self.gParam.sPosSelf+1] == 1 then
			local s = param.zhuangPos == self.gParam.sPosSelf and "坐" or "拉"
			local str = s .. param.maNum1[self.gParam.sPosSelf+1] .. "跑" .. param.maNum2[self.gParam.sPosSelf+1]
			self.UIHeadLayout:setZuoLaPaoTxt(self.gParam.sPosSelf,str)

			if self.mUIBetMultiple and (not tolua.isnull(self.mUIBetMultiple)) then
				self.mUIBetMultiple:removeFromParent()
				self.mUIBetMultiple = nil
			end
		elseif param.maState[self.gParam.sPosSelf+1] == 0 then
			local args = {
		        isZhuang  = self.gParam.sPosSelf == param.zhuangPos,
		        diFen     = gRoomData:getRoomInfo().diFen,
		        countDown = param.countDown,
			}
			if self.gParam.roomType ~= DefineRule.RoomType.CoinRoom then
				args.countDown = 0
			end
			if (not self.mUIBetMultiple) or tolua.isnull(self.mUIBetMultiple) then
				self.mUIBetMultiple = require("app.Game.UI.UIGame.MJHuaiBei.UIBetMultipleEx"):create(args)
			 	self:addChild(self.mUIBetMultiple,ConfigGameScene.ZOrder.PLAYER_INFO_TIPS+1)
			 	-- self.mUIBetMultiple:setPosition(display.cx,display.cy)
			end
		end
	else
		dump(param,"==setZuoLaPao==" .. self.gParam.sPosSelf)
		for i,v in ipairs(param.maNum1) do
			local s = (param.zhuangPos +1) == i and "坐" or "拉"
			local str = s .. v .. "跑" .. param.maNum2[i]
			self.UIHeadLayout:setZuoLaPaoTxt(i-1,str)
		end
