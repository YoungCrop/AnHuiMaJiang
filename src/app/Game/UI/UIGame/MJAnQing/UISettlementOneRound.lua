...
function m:printArgs()
    dump(self.param,self.__TAG .. "===UISettlementOneRound=")
end
...
function m:loadCSB()
    ...
    for i,v in ipairs(self.param.playmes.m_playtype) do
        ...
        if DefineRule.GRDiHua[v] then
            self.diHua = DefineRule.GRDiHua[v]
        end
    end
    ...
    if cc.exports.gGameConfig.isiOSAppInReview then
        ...
        local posX = csbNode:getContentSize().width*0.5
        self.nodeMap["btnNext"]:setPositionX(posX)
    end
...
function m:handleData()
    ...
    for i = 1, #self.param.roomPlayers do
        ...
        local txtHuType = "" --"花数翻倍"
        for n = 1, #self.param.msgTbl["m_huScore" .. i] do
            local huTypeIndex = self.param.msgTbl["m_huScore" .. i][n][1]
            txtHuType = txtHuType .. DefineHuPai.huTpyeStr[huTypeIndex] .. ""
            local score = math.abs(self.param.msgTbl["m_huScore" .. i][n][2])
            if n == #self.param.msgTbl["m_huScore" .. i] then
                txtHuType = txtHuType .. score .. ")"
            else
                txtHuType = txtHuType .. score .. plus
            end
        end
        local str = ""
        if txtHuType ~= "" then
            if self.diHua then
                str = str .. "花数翻倍" .. self.diHua.huaShu .. "*("
            end
            if data.score < 0 then
                str = str .. plus
            end
            str = str .. txtHuType
            str = str .. plus .. "底分" .. self.param.msgTbl.m_bomb[i]
        end
        data.txtHuType = str
