...
local SceneMJAnQing = require("app.Game.Scene.SceneMJAnQingEx")
...
function m:roomAddPlayer(roomPlayer)
    ...
    gRoomData:setUIPosBySPos(roomPlayer.seatIdx-1,roomPlayer.displayIdx,self.gamePlayerNum)
    -- 玩家信息
...
function m:playOutMjTile(seatIdx, mjColor, mjNumber)
    ...
    self:showOutMjTileAnimation(roomPlayer, mjColor, mjNumber, function()
        ...
        -- 显示出牌标识
        self:showOutMjtileSign(roomPlayer,mjColor, mjNumber)
    end)
...
function m:outMjTile(roomPlayer, mjColor, mjNumber)
    ...
    mjTile.mjColor = mjColor
    mjTile.mjNumber = mjNumber
    if self:IsHuaCard(mjColor, mjNumber) then
...
-- @param mjNumber 编号
-- end --
function m:addMjTilePung(seatIdx, mjColor, mjNumber,srcSPos)
    local roomPlayer = self.roomPlayers[seatIdx]
    ...
    local op = gRoomData:getOprateCodeName(seatIdx-1,srcSPos)
    pungData.groupNode = self:pungBarReorderMjTiles(roomPlayer, mjColor, mjNumber,op, false)
end
...
-- end --
function m:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar,srcSPos)
    local roomPlayer = self.roomPlayers[seatIdx]
    ...
    if isBrightBar then
        -- 明杠
        op = gRoomData:getOprateCodeName(seatIdx-1,srcSPos or 5)
        table.insert(roomPlayer.mjTileBrightBars, barData)
    else
        -- 暗杠
        table.insert(roomPlayer.mjTileDarkBars, barData)
    end
    log("addMjTileBar---12345=====",seatIdx, mjColor, mjNumber, isBrightBar,srcSPos,op)
    barData.groupNode = self:pungBarReorderMjTiles(roomPlayer, mjColor, mjNumber,op, true, isBrightBar)
end
...
function m:addMjTileSpecialBar(seatIdx, cards, type)
    ...
    if not roomPlayer.mjTileSpecialBars[type].cards then
        ...
        local op = gRoomData.opCodeEnum.SelfOp
        barData.groupNode = self:pungBarReorderMjTiles(roomPlayer, nil, nil,op, true, 3, cards,type)
    else
...
-- @return
-- end --
function m:pungBarReorderMjTiles(roomPlayer, mjColor, mjNumber, opCode,isBar, isBrightBar,cards,typeFlag)
    log("pungBarReorderMjTiles===",mjColor, mjNumber, opCode)
    local groupNode = nil
    local isEat = false
...
function m:showDecisionAnimation(seatIdx, decisionType)
        ...
        local index = SceneMJAnQing.SoundDecisionType[decisionType]
        if index then
...
-- @param seatIdx 座次
-- end --
function m:showOutMjtileSign(roomPlayer,mjColor, mjNumber)
    -- local roomPlayer = self.roomPlayers[seatIdx]
    local endIdx = #roomPlayer.outMjTiles
    local outMjTile = roomPlayer.outMjTiles[endIdx]

    if mjColor and mjNumber and self:IsHuaCard(mjColor, mjNumber) then
        outMjTile = roomPlayer.outMjTilesHua[#roomPlayer.outMjTilesHua]
    end
    if outMjTile then
