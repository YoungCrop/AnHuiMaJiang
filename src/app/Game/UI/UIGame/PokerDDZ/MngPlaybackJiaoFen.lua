...
local m = class("MngPlaybackJiaoFen")
...
m.VIDEOOPER = {
    ...
    VIDEO_OPER_JIAO             = 201,--叫分;
    VIDEO_OPER_BUJIAO           = 200,--不叫分;

    VIDEO_OPER_CHUNTIAN         = 50, --春天
    VIDEO_OPER_FANCHUN          = 51, --玩家反春;

    VIDEO_OPER_JIABEI           = 30 , -- 加倍
    VIDEO_OPER_BUJIABEI           = 31 , -- 加倍
    VIDEO_OPER_GEN           = 32 , -- 加倍
    VIDEO_OPER_BUGEN          = 33 , -- 加倍
...
function m:ctor(rootNode, paramTbl, gameType)
    dump( paramTbl , "[log_zxh] MngPlaybackJiaoFen paramTbl")
    ...
    local grabNode = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_Fen")
    grabNode:setVisible(false)

    gComm.UIUtils.seekNodeByName(self.rootNode, "Node_jiabei"):setVisible(false)
    gComm.UIUtils.seekNodeByName(self.rootNode, "Node_bujiabei"):setVisible(false)
    gComm.UIUtils.seekNodeByName(self.rootNode, "Node_gen"):setVisible(false)
    gComm.UIUtils.seekNodeByName(self.rootNode, "Node_bugen"):setVisible(false)
    ...
    self.bomptimes = 1
    for i=1,3 do
        local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..i)
        log("[log_zxh] bombtimes 1 :"..self.bomptimes)
        nd:setString(tostring(self.bomptimes))
    end

    self.bombNum = 0
    for i,v in ipairs(paramTbl.m_playtype) do
        if v == 125 or v == 126 or v == 127 then
            self.bombLimit = v
        end
    end
    ...
    log("[log_zxh] boomLimit:"..self.bombLimit)
...
function m:roomAddPlayer(roomPlayer)
    ...
    -- 添加玩家是否加倍 跟状态
    roomPlayer.addDouble = false
    roomPlayer.addGen = false
    roomPlayer.isDiZhu = false
    roomPlayer.bomptimes = 1
    -- 添加入缓冲
...
function m:ScoreEvery(v ,curStep , bombCount ,isJiaoQiang,isChuan)
    log("[log_zxh] bombCount:"..bombCount)
    if isJiaoQiang then
        v.bomptimes = bombCount
    else

        if isChuan then
            v.bomptimes = v.bomptimes * math.pow(2, bombCount)
        elseif self.bombNum < self.bombLimit then
            v.bomptimes = v.bomptimes * math.pow(2, bombCount)
        end
    end
    self.bombs[v.displayIdx][curStep] = v.bomptimes
end


function m:ScoreNum(v ,curStep , bombCount,isJiaoQiang,isChuan)
    self:ScoreEvery(v ,curStep ,bombCount,isJiaoQiang,isChuan)
end
...
function m:playOperation(seatIdx, optType, card, isPre,curStep,isLast)
    ...
    elseif optType == m.VIDEOOPER.VIDEO_OPER_BUJIAO then --不叫
        self:playVoice(1,roomPlayer.sex,isPre)
        self:showOperationTips("Node_grab","Image_nograb_",nil,realSeat)

    elseif optType == m.VIDEOOPER.VIDEO_OPER_JIAO then -- 叫分

        --[[local step = self.bombs[curStep]
        if step then
            self.bomptimes = step
        else
            self:ScoreNum(card[1],true)
        ...
        for i=1,3 do
            local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..i)
            log("[log_zxh] bombtimes 2 :"..self.bomptimes)
            nd:setString(tostring(self.bomptimes))
        end--]]
        log("[log_zxh] setPlayerBomptimes 11")
        self:setPlayerBomptimes(curStep , card , true)

        local grabNode = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_Fen")
        grabNode:setVisible(false)

        if card[1] == 1 then
            self:showOperationTips("Node_Fen","Image_one_","Image_one_",realSeat)
        elseif card[1] == 2 then
            self:showOperationTips("Node_Fen","Image_two_","Image_two_",realSeat)
        elseif card[1] == 3 then
            self:showOperationTips("Node_Fen","Image_three_","Image_three_",realSeat)
        end
    ...
    elseif optType == m.VIDEOOPER.VIDEO_OPER_ROCKET
        ...
        log("[log_zxh] setPlayerBomptimes 22")
        self:setPlayerBomptimes(curStep , nil , 1)
        ...
        log("m:VIDEO_OPER_BOMB2",self.bomptimes)

        for i=1,3 do
            local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..i)
            log("[log_zxh] bombtimes 3 :"..self.bomptimes)
            nd:setString(tostring(self.bomptimes))
        end--]]
    ...
    elseif optType == m.VIDEOOPER.VIDEO_OPER_CHUNTIAN then
        ...
        for i=1,3 do
            local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..i)
            log("[log_zxh] bombtimes 4 :"..self.bomptimes)
            nd:setString(tostring(self.bomptimes))
        end --]]

        self:setPlayerBomptimes(curStep , 1 , nil , true)
    elseif optType == m.VIDEOOPER.VIDEO_OPER_FANCHUN then
        ...
        for i=1,3 do
            local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..i)
            log("[log_zxh] bombtimes 5 :"..self.bomptimes)
            nd:setString(tostring(self.bomptimes))
        end--]]
        log("[log_zxh] setPlayerBomptimes 44")

        self:setPlayerBomptimes(curStep , 1 , nil , true)

    elseif optType == m.VIDEOOPER.VIDEO_OPER_JIABEI then
        -- 判断是否是地主加倍
        local isDiZhuDouble = false
        for i,roomPlayer in ipairs(self.roomPlayers) do
            if roomPlayer.seatIdx == seatIdx and roomPlayer.isDiZhu then
                isDiZhuDouble = true
            end
        end

        for i,v in ipairs(self.roomPlayers) do
            local roomPlayer = v
            local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..roomPlayer.displayIdx)
            -- log("[log_zxh] bombtimes 6 :"..self.bomptimes)
            local bomptimes = roomPlayer.bomptimes
            log("[log_zxh] playerSeatIdx:"..roomPlayer.seatIdx.."   seatIdx:"..seatIdx)

            if roomPlayer.seatIdx == seatIdx then
                log("=======================")
                bomptimes = bomptimes * 2
                roomPlayer.addDouble = true
            end

            if isDiZhuDouble then
                if (not roomPlayer.isDiZhu and roomPlayer.addDouble )
                    or (not roomPlayer.isDiZhu and roomPlayer.addGen) then
                    bomptimes = roomPlayer.bomptimes * 2
                end
            end

            -- 加倍
            if roomPlayer.isDiZhu and
                roomPlayer.seatIdx ~= seatIdx  then
                log("***********dizhu************")
                -- 自己是地主
                bomptimes = roomPlayer.bomptimes * 2
            end

            roomPlayer.bomptimes = bomptimes
            log("~~~~~~~~[log_zxh] double  bomptimes:"..roomPlayer.bomptimes)
            nd:setString(tostring(roomPlayer.bomptimes))
        end

        -- 加倍
        self:playVoice(5,roomPlayer.sex,isPre)
        self:showOperationTips("Node_jiabei","spr_jiabei_",nil,realSeat)
    elseif optType == m.VIDEOOPER.VIDEO_OPER_BUJIABEI then
        -- 不加倍
        self:playVoice(6,roomPlayer.sex,isPre)
        self:showOperationTips("Node_bujiabei","spr_bujiabei_",nil,realSeat)
    elseif optType == m.VIDEOOPER.VIDEO_OPER_GEN then
        -- 跟
        for i,v in ipairs(self.roomPlayers) do
            local roomPlayer = v
            local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..roomPlayer.displayIdx)
            ...
            if roomPlayer.seatIdx == seatIdx then
                -- 当前玩家点击加倍
                bomptimes = bomptimes * 2
                roomPlayer.addGen = true
            end

            --[[if roomPlayer.isDiZhu then
                bomptimes = bomptimes * 2
            end--]]

            roomPlayer.bomptimes = bomptimes
            log("********[lsog_zxh] Gen bomptimes:"..roomPlayer.bomptimes)
            nd:setString(tostring(bomptimes))
        end

        self:playVoice(7,roomPlayer.sex,isPre)
        self:showOperationTips("Node_gen","spr_gen_",nil,realSeat)
    elseif optType == m.VIDEOOPER.VIDEO_OPER_BUGEN then
        -- 不跟
        self:playVoice(8,roomPlayer.sex,isPre)
        self:showOperationTips("Node_bugen","spr_bugen_",nil,realSeat)
    else
        self:plauOutCard(roomPlayer,seatIdx,card)
    end
end
    ...
    for i=1,3 do
        ...
        if realSeat == i then -- 地主
            ...
            if roomPlayer.isOneself and self.gameType ~= 1 then
                ...
                log("[log_zxh] bombtimes 6 :"..self.bomptimes)
                nd:setString(tostring(self.bomptimes))--]]
                -- log("[log_zxh] setPlayerBomptimes 55")

                -- self:setPlayerBomptimes(curStep , 1 , nil ,true)


            end
            roomPlayer.isDiZhu = true
...
function m:showOperationTips(fstLayer,sndLayer,other,realSeat)
    ...
    local Node_Fen = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_Fen")
    for _,v in pairs(Node_Fen:getChildren()) do
        v:setVisible(false)
    end

    local Node_jiabei = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_jiabei")
    for _,v in pairs(Node_jiabei:getChildren()) do
        v:setVisible(false)
    end

    local Node_bujiabei = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_bujiabei")
    for _,v in pairs(Node_bujiabei:getChildren()) do
        v:setVisible(false)
    end

    local Node_gen = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_gen")
    for _,v in pairs(Node_gen:getChildren()) do
        v:setVisible(false)
    end

    local Node_bugen = gComm.UIUtils.seekNodeByName(self.rootNode, "Node_bugen")
    for _,v in pairs(Node_bugen:getChildren()) do
        v:setVisible(false)
    end
...
function m:playSound(seatIdx, optType, card)
    ...
    elseif optType == m.VIDEOOPER.VIDEO_OPER_QIANGDIZHU then
        if roomPlayer.sex == 1 then
            sound = "man/rob"
        else
            sound = "woman/rob"
        end
    elseif optType == m.VIDEOOPER.VIDEO_OPER_BUQIANG then
        if roomPlayer.sex == 1 then
            sound = "man/norob"
        else
            sound = "woman/norob"
...
-- 设置玩家炸弹数量
function m:setPlayerBomptimes(curStep ,card ,param1 , param2)
    for i,v in ipairs(self.roomPlayers) do
        local step = nil
        if self.bombs[v.displayIdx] then
            step = self.bombs[v.displayIdx][curStep]
        end
        if step then
            v.bomptimes = step
        else
            if card then
                if type(card) == "table" then
                    self:ScoreNum(v ,curStep , card[1],param1,param2)
                else
                    self:ScoreNum(v ,curStep , card,param1,param2)
                end
            else
                self:ScoreNum(v , curStep ,param1 , param2)
            end
        end

        local nd = gComm.UIUtils.seekNodeByName(self.rootNode, "AtlasLabel_bombtimes_"..v.displayIdx)
        log("==========[log_zxh] bomptimes:"..v.bomptimes)
        nd:setString(tostring(v.bomptimes))
    end
    -- local step = self.bombs[curStep]
    -- if step then
    --  self.bomptimes = step
