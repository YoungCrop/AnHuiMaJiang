...
--设置按钮点击
function m.setButtonClick(_btn,_handler,_sound)
    local __cname = tolua.type(_btn)
    print("BtnUtils__cname==",__cname)
...
function m.showCollector(_node, _time, _pos, _callback)
...
function m.playParticleFile(_particleFile,_parentNode,_pos,_zOrder, _isAutoRemove,_playTime)
...
function m.setCellButtonClick(_btn,_handler)
    local function onTouchBegan(touch, event)
...
function m.getTimeTableBySceond2(_seconds)
...
--设置ControlButton显示图片
function m.setControlButtonImage(_button,_frameName)
...
--设置ControlButton显示图片
function m.setControlButtonState(_button)
...
function m.setClip(_path,_parent)
        --头像
...
function m.showTipLabel(_value)
...
--显示不完整，进行滚动显示label
function m.setLabelScrollTextWx(_label, _value)
