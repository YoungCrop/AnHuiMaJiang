...
local MngPlayback = require("app.Game.UI.UIGame.MJAnQing.MngPlayback")
...
local csbFile = "Csd/SubGame/MJAnQing/UIPlayback.csb"
function m:ctor(replayData)
    m.super.ctor(self)
    dump(replayData,"=m:ctor(replayData)=",9)
...
-- [LUA-print] -         }
        ...
        local srcPos  = replayStepData[5] and replayStepData[5][1] or 5--碰杠位置
        ...
        elseif optType == 5 then
            -- 碰
            self.Manager:addMjTilePung(seatIdx, mjColor, mjNumber,srcPos)

            self.Manager:removePrePlayerOutMjTile()
        elseif optType == 6 or optType == 18 then
            -- 别人打的牌,自己可以明杠之
            self.Manager:addMjTileBar(seatIdx, mjColor, mjNumber, true,srcPos)
