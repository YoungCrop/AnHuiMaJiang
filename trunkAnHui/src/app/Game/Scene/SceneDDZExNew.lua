...
local UIRoomInfo = require("app.Game.UI.UIIRoom.RoomComm.UIRoomInfo")
...
local m = class("SceneDDZExNew", gComm.SceneBase)
...
local csbRes = "Csd/Scene/SceneDDZ.csb"
...
function m:ctor(enterRoomMsgTbl)
	...
	self.UIRoomInfo = UIRoomInfo:create(param)
	...
	if agentRoom == 1 then
		self.UIRoomInfo:setBtnBackLobby(true)
	end
...
function m:onRcvSyncRoomState(msgTbl,isStart)
	...
	if msgTbl.m_state == 1 then
		...
		if self.isRoomCreater then
			self.UIRoomInfo:setBtnBackLobby(false)
	 	else
			self.UIRoomInfo:setBtnBackLobby(true)
	 	end
		return
	...
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		...
		if self.gParam.roomType == DefineRule.RoomType.BigMatch and msgTbl.m_IsTuoguan then
			...
			if self.gParam.sPosSelf == seatIdx-1 then
				...
				self.UIRoomInfo:setTuoGuanBtnShow(not isTuoGuan)
			else
...
function m:onRcvReady(msgTbl)
	...
	if seatIdx == self.playerSeatIdx then
		...
		self.UIRoomInfo:setRoundIndex(str)
...
function m:onRcvRoundState(msgTbl)
	...
	self.curCircle = msgTbl.m_curCircle + 1
...
function m:onRcvChatMsg(msgTbl)
	...
	if msgTbl.m_type == 4 then
		...
		if msgTbl.m_type == DefineRoom.ChatType.FIX_MSG then
			...
			msgLabel:setString(mTxtTipConfig.GetConfigTxt(textStr))
			isTextMsg = true
...
function m:onError( socket ,errorInfo)
	gComm.UIUtils.showLoadingTips(mTxtTipConfig.GetConfigTxt("LTKey_0001"))
	socket:close()
	self:reLogin()
...
-- //1.进入托管  2.取消托管
		...
		self.UIRoomInfo:setTuoGuanBtnShow(flag)
