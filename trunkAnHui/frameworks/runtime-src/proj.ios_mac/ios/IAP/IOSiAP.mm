...
void IOSiAP::removeProduct(std::string &identifier)
{

    std::vector <IOSProduct *>::iterator iterator;
    for (iterator = iOSProducts.begin(); iterator != iOSProducts.end();) {
        IOSProduct *iosProduct = *iterator;
        
        if (identifier == iosProduct->productIdentifier)
        {
            delete iosProduct;
            iterator = iOSProducts.erase(iterator);
        }
        else
        {
            iterator++;
